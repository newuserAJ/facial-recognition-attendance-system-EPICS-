...
def TrackImages():
    ...
    # Clear previous data from treeview (TV)
    ...
    # ✅ STEP 1: Establish Serial connection with Arduino
    try:
        arduino = serial.Serial('COM12', 9600)
        time.sleep(2)  # Wait for Arduino to initialize
    except Exception as e:
        mess._show(title='Arduino Error', message='Could not connect to Arduino: ' + str(e))
        arduino = None
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            serial_num, conf = recognizer.predict(gray[y:y + h, x:x + w])

            if conf < 50:
                ts = time.time()
                date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
                timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                aa = df.loc[df['SERIAL NO.'] == serial_num]['NAME'].values
                ID = df.loc[df['SERIAL NO.'] == serial_num]['ID'].values
                ID = str(ID)[1:-1]
                bb = str(aa)[2:-2]
                attendance = [str(ID), '', bb, '', str(date), '', str(timeStamp)]

                # ✅ STEP 2: Send '1' to Arduino (face recognized)
                if arduino:
                    arduino.write(b'1')
                ...
                # ✅ STEP 3: Send '0' to Arduino (unknown face)
                if arduino:
                    arduino.write(b'0')
    ...
    # ✅ STEP 4: Close Arduino connection
    try:
        if arduino:
            arduino.close()
    except:
